...
Path: ``$MAELCOM_AUTH`` or ``maelcom.auth.toml`` in the working directory.
"""
...
# Source kinds the Settings tab exposes (endpoint is optional per kind).
AUTH_SOURCES = ("github", "gitlab", "jira", "slack", "pagerduty")


def auth_path() -> Path:
    return Path(os.environ.get("MAELCOM_AUTH", "maelcom.auth.toml"))
