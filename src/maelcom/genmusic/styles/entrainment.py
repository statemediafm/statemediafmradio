...
_PHASE_BARS = 36  # ~72 s per phase → ~12–15 min journeys
...
def _frame_layers(hz: float, i: int, seed: int) -> list[str]:
    ...
    verb = 0.85
    # A continuous low drone (root + fifth), slow filter breath, slow spatial pan.
    drone = (
        f'    note("<[a1,e2]>").s("sine").attack(4).release(6)'
        f".lpf(sine.range(170,430).slow(24)).pan(sine.range(0.35,0.65).slow(44))"
        f".room({verb}).roomsize(9).gain(0.3)"
    )
    ...
    if mode == 0:  # isochronic amplitude throb
        carrier = (
            ...
            f".gain(sine.range(0,0.13).fast({n})).room(0.7).roomsize(8)"
        )
        ...
        carrier = (
            ...
            f".attack(3).release(4).room(0.75).roomsize(9).gain(0.14)"
        )
    else:  # PAN holds the tempo — a spatial pulse at the band rate
        carrier = (
            f'    freq({_CARRIER:g}).s("sawtooth").lpf(360)'
            f".pan(sine.range(0.1,0.9).fast({n})).gain(0.1).room(0.7).roomsize(8)"
        )
    # A soft modal pad — ducks anti-phase to the noise wave (they trade presence).
    pad = (
        f'    n("<[0,2,4] [0,3,4] [0,2,5]>").scale("{_pent(2)}").s("sawtooth")'
        f".attack(3).release(5).lpf(sine.range(280,600).slow(30))"
        f".pan(sine.range(0.6,0.4).slow(52)).room(0.85).roomsize(9)"
        f".gain(sine.range(0.16,0.05).slow(30))"
    )
...
def _m_binaural(i: int, seed: int, hz: float) -> str:
    ...
    return (
        f'    stack(freq({_CARRIER:g}).s("sine").pan(0).attack(4).release(4).gain(0.15), '
        f'freq({fr:g}).s("sine").pan(1).attack(4).release(4).gain(0.15))'
    )
...
def _m_sub_wave(i: int, seed: int, hz: float) -> str:
    ...
    return (
        ...
        f".gain(sine.range(0.08,0.32).slow(28))"
    )
...
def _m_noise_wave(i: int, seed: int, hz: float) -> str:
    ...
    return (
        ...
        f".pan(sine.range(0.7,0.3).slow(46)).gain(sine.range(0.02,0.13).slow(30))"
    )
...
def _m_rainstick(i: int, seed: int, hz: float) -> str:
    ...
    return (
        ...
        ".gain(rand.range(0,0.05).fast(20)).room(0.72).roomsize(8)"
    )


def _m_chime(i: int, seed: int, hz: float) -> str:
    """A sparse bright chime with a long echo (hypnotic delay) and wide reverb."""
    deg = _pick(seed, i, "chime", 5)
    return (
        f'    n("<~ ~ ~ {deg} ~ ~ ~ ~>").scale("{_pent(4)}").s("sine")'
        f".attack(0.005).release(3).delay(0.7).delaytime(0.75).delayfeedback(0.55)"
        f".pan(sine.range(0.25,0.75).slow(34)).room(0.9).roomsize(9).gain(0.11)"
    )


def _m_water(i: int, seed: int, hz: float) -> str:
    """A water drop — a short high plink with a quick rising pitch, echoed."""
    return (
        f'    n("~ ~ [7 8] ~ ~ ~ ~ ~").scale("{_pent(5)}").s("sine")'
        f".attack(0.002).decay(0.09).sustain(0).release(0.25)"
        f".delay(0.4).delaytime(0.5).delayfeedback(0.4).pan(rand).room(0.85).gain(0.09)"
    )


def _m_arp(i: int, seed: int, hz: float) -> str:
    """A gentle plucked basso-continuo arpeggio — warm, low-passed, lightly delayed."""
    return (
        f'    n("0 ~ 2 ~ 4 ~ 2 ~").scale("{_pent(3)}").s("sawtooth")'
        f".attack(0.005).decay(0.3).sustain(0.08).release(0.6).lpf(720)"
        f".delay(0.3).delaytime(0.375).delayfeedback(0.35).room(0.75).gain(0.11)"
    )


def _m_drum(i: int, seed: int, hz: float) -> str:
    """A rare, soft low drum motif — a reverbed heartbeat, never a snare."""
    return (
        '    note("<a1 ~ ~ ~ ~ ~ a1 ~>").s("sine").attack(0.002).decay(0.45)'
        ".sustain(0).release(0.3).lpf(190).room(0.6).roomsize(7).gain(0.22)"
    )
...
def render(signal: ActivitySignal, intensity: float, band: str, fade_ms: int = 2000) -> str:
    ...
    seed = _seed(signal)
    n = 11 + (seed % 4)  # 11–14 phases → ~13–17 min
